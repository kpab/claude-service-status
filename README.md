...
- 🖱️ **Hover for details** — A tooltip shows the overall status, ongoing incidents, and the state of each component.
...
> ℹ️ The Statuspage v2 API is rate-limited to 1 request per second per IP, so a refresh interval of 15 seconds or more is recommended.
